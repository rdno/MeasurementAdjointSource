...
obsd_dir = "../adjsrc_fortran/sac"
synt_dir = "../adjsrc_fortran/sac"
quakeml = "C200912191302A.xml"
stationxml = "IU.AFI.xml"

src_type = "multitaper_misfit"
tag_type = "mt"

config = pyadjoint.ConfigMultiTaper(min_period=45.0, max_period=110.0,
                          lnpt=15,
...
for catlog in ("dt", "am"):
    ...
    # Z
    fobsd = os.path.join(obsd_dir, "AFI.IU.LHZ.obs.0.sac")
    fsynt = os.path.join(synt_dir, "AFI.IU.MXZ.sem.0.sac")

    obs = read(fobsd)
    syn = read(fsynt)

    fadjsrc_z = "IU.AFI..LHZ.%s.%s.adj" % (tag_type, config.measure_type)
    window = [[816, 1067]]
    ...
    interp_starttime = adjsrc_z.starttime
    interp_delta = adjsrc_z.dt
    interp_npts = len(adjsrc_z.adjoint_source)


    adjsrc_final = adjsrc_all
    # adjsrc_final =\
